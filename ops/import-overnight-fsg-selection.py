#!/usr/bin/env python3
"""Import selected CC0 Smithsonian objects for uniquely matched existing artists."""
...
fsg=module('fsg','overnight-fsg-images.py');guard=module('import_guard','import-overnight-met-selection.py');core=fsg.core
SOURCE='overnight-fsg-selected-primary-20260915';INSTITUTION='smithsonian-national-museum-of-asian-art'

def candidate(lead):
    o=lead['object'];artist=lead['artist'];oid=lead['source_object_id'];aid=str(uuid.uuid5(uuid.NAMESPACE_URL,'https://artline.local/night-selected-fsg/'+oid));dn=o['content']['descriptiveNonRepeating'];ft=o['content']['freetext'];page=dn['record_link']
    c={'artwork_id':aid,'slug':'night-fsg-'+oid,'external_id':oid,'scheme':'fsg-object','provider':'night-fsg','title':lead['title'],
       **{k:lead[k] for k in ('date_display','creation_year_start','creation_year_end','date_precision','work_type','accession_number')},
       'creation_place_display':lead['creation_place_display'],'artist':artist['display_name'],'artist_id':artist['id'],'artist_slug':artist['slug'],'artist_slugs':[artist['slug']],'artist_qid':artist['qid'],
       'aliases':artist['aliases'],'roles':['primary'],'popular':artist['popular'],'page':page,'qid':None,'target_ids':{'local':aid},
       'raw':{'object':o,'metadata_capture':lead['metadata_capture']}}
    image,url,facts=fsg.source_match(c,o);credit=c['artist']+'; '+facts['source_collection']+', Smithsonian Institution, Washington, DC; '+('; '.join(fsg.fields(ft,'creditLine')) or 'National Museum of Asian Art')+'; '+c['accession_number'];checked=lead['metadata_capture']['retrieved_at']
    c.update(source_image_url=url,scope_evidence=facts,policy_url=fsg.CC0,rights_status='cc0',license_label='CC0 1.0',checked_at=checked,
      creator_credit=credit,attribution_text=c['artist']+'. '+c['title']+'. '+credit+'. CC0 1.0 ('+fsg.CC0+'). Full-frame proportional resize and JPEG compression.',
      source_name='National Museum of Asian Art',source_record_url=page,image_url=url,image_license='CC0 1.0',image_license_url=fsg.CC0,rights_statement='CC0',creator=c['artist'],creation_date=c['date_display'],source_object_id=oid,rights_verified_at=checked,metadata_license=fsg.CC0,
      medium_text='; '.join(fsg.fields(ft,'physicalDescription','Medium')) or None,dimensions_text='; '.join(fsg.fields(ft,'physicalDescription','Dimensions')) or None)
    return c
...
def build(run,reference,limit):
    ...
    for lead in json.loads((reference/'source-leads.json').read_text()):
        rows.append(candidate(lead))
    with fsg.ro('postgres://localhost/artline') as db:state=guard.snapshot(db,rows,INSTITUTION,['fsg-object']);selected,held=guard.conflicts(rows,state)
    ...
    data={'at':core.now(),'records':selected,'held':held,'policy':'Official Smithsonian source records and exact per-image CC0; unique existing creator name/alias and non-conflicting source biography. Museum holdings, dates and type verified. All additions remain research candidates in review; no artist nationality inferred.'}
    ...
    print('FSG selected',len(selected),'popular',sum(c['popular'] for c in selected),'types',dict(collections.Counter(c['work_type'] for c in selected)),'held',len(held),flush=True);return data
...
def apply(run,target):
    ...
    with psycopg.connect(dsn,autocommit=True,row_factory=dict_row) as db:
        state=guard.snapshot(db,records,INSTITUTION,['fsg-object']);selected,held=guard.conflicts(records,state);iid=state['institution_id']
        ...
        with db.transaction():
            db.execute("INSERT INTO sources(slug,name,source_type,base_url,terms_url) VALUES(%s,'National Museum of Asian Art: selected independently verified works','museum_api','https://asia.si.edu/',%s) ON CONFLICT(slug) DO NOTHING",(SOURCE,fsg.CC0));sid=db.execute('SELECT id FROM sources WHERE slug=%s',(SOURCE,)).fetchone()['id']
        for start in range(0,len(selected),25):
            ...
            with db.transaction():
                ...
                if active:
                    collisions=db.execute("SELECT entity_id FROM external_identifiers WHERE entity_type='artwork' AND ((scheme='fsg-object' AND external_id=ANY(%s)) OR canonical_url=ANY(%s))",([c['external_id'] for c in active],[c['page'] for c in active])).fetchall();assert not collisions,'Source identity appeared during import'
                with db.pipeline():
                    for c in group:
                        fsg.source_match(c,c['raw']['object'])
                        ...
                        db.execute("INSERT INTO external_identifiers(entity_type,entity_id,scheme,external_id,canonical_url,source_id,retrieved_at) VALUES('artwork',%s,'fsg-object',%s,%s,%s,%s)",(aid,c['external_id'],page,sid,checked))
                        db.execute("INSERT INTO artwork_location_assertions(artwork_id,claim_type,institution_id,context,source_id,source_url,evidence_note,checked_at,review_state) VALUES(%s,'holding',%s,'collection',%s,%s,%s,%s,'accepted')",(aid,iid,sid,page,'Official National Museum of Asian Art Collection record, accession '+c['accession_number']+'. Holding only; no current display claim.',checked))
                        evidence={'primary_record':c['raw']['object'],'metadata_capture':c['raw']['metadata_capture'],'metadata_license':fsg.CC0,'creator_identity':{'existing_wikidata':c['artist_qid'],'existing_slug':c['artist_slug'],'museum_artist_field':c['scope_evidence']['source_creator']},'source_origin':c['creation_place_display'],'date_review':'Original source date and explicit uncertainty retained; no lifespan-derived creation dates.'}
            ...
            print(core.now(),target,'FSG metadata',len(out),'of',len(selected),flush=True)
    ...
    if target=='local':
        for c in records:core.save_new(run/'selected/night-fsg'/(c['artwork_id']+'.json'),c)
