...
RUN = ROOT / 'docs/research/cypriot-expansion-20260914'
BACKUP = Path('/Users/vadimdulub/Library/Application Support/Artline/backups/cypriot-expansion-20260914')
...
SOURCE = 'cypriot-expansion-research-20260914'
SITE = 'https://artline-web-lpuqqlugnq-ew.a.run.app'

def uid(key): return str(uuid.uuid5(uuid.NAMESPACE_URL, 'https://artline.local/cypriot-expansion/' + key))
...
def research():
    raise SystemExit('Research is immutable selected.json; see research-cypriot-expansion.py and retained source captures.')
...
def images():
    ...
    for work in selected['works']:
        ...
        content,width,height,quality=core.compress(raw);checksum=core.sha(content);path='/assets/artworks/imported/cypriot-expansion/'+work['key']+'-'+checksum[:16]+'.jpg'
...
def plan():
    ...
    for target in ('local','production'):
        with w.base.connect(target=='production') as db,db.transaction():
            ...
            found=db.execute("SELECT a.id::text,a.title,a.unlinked_creator_label FROM artworks a WHERE EXISTS(SELECT 1 FROM citations c WHERE c.entity_type='artwork' AND c.entity_id=a.id AND c.source_url=ANY(%s)) OR (lower(a.title)=ANY(%s) AND a.unlinked_creator_label ~* 'Savva|Kanth|Micha|Ioann|Kash|Econom|Averk|Frag|Parask|Güney|Guney|Minas|Goul|Axent|Afxent')",(urls,titles)).fetchall();assert not found,found
