...
spec=importlib.util.spec_from_file_location('r',Path(__file__).with_name('research-russian-deep-images.py'))
...
def main(label):
 ...
 quality=RUN/'quality-review/final-image.json'
 if quality.exists():
  q=json.loads(quality.read_bytes());assert (RUN/'quality-review/database-receipt.json').exists()
  selected=[q if s['artwork_id']==q['artwork_id'] else s for s in selected];receipts=[q if s['artwork_id']==q['artwork_id'] else s for s in receipts]
 ...
 for target,dsn in [('local','postgres://127.0.0.1/artline'),('production',core.cloud_dsn())]:
  ...
  with psycopg.connect(dsn,row_factory=dict_row) as db:
   ...
   rows=db.execute("""WITH input AS(SELECT * FROM jsonb_to_recordset(%s::jsonb) AS x(scheme text,external_id text))
    SELECT e.scheme,e.external_id,a.id::text artwork_id,a.slug,a.title,a.alternate_title,a.accession_number,a.date_display,a.creation_year_start,a.creation_year_end,a.date_precision,a.work_type,a.object_form,a.cultural_context,a.unlinked_creator_label,a.status,a.primary_media_id::text,i.slug institution,
   ...
   for s in completed:
    ...
    for key in ('slug','title','alternate_title','accession_number','date_display','creation_year_start','creation_year_end','date_precision','work_type','object_form','cultural_context','unlinked_creator_label','status','institution'):assert x[key]==b[key],(target,s['external_id'],key)
    assert sorted(x['creators'] or [],key=lambda a:(a['id'],a['role']))==sorted(b['creators'] or [],key=lambda a:(a['id'],a['role']))
   ...
   if (RUN/'commons-research-database-receipt.json').exists():
    candidate_evidence=db.execute("SELECT count(*) n FROM citations c JOIN sources s ON s.id=c.source_id WHERE s.slug='russian-deep-image-research' AND c.field_name='image_candidate'").fetchone()['n']
    assert candidate_evidence==len(selected)
