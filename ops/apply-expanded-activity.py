...
def load(directory):
 ...
 for r in rows:
  ...
  if common.QUALIFIED.search(p['name']) or re.search(r'\b(master|mestari|mästaren)\b|monogram',p['name'],re.I) or a['entity_type']!='person':raise ValueError('Unresolved creator')
  if a['new']:
   if a.get('timeline_basis')=='activity':
    first,last=a.get('active_start_year'),a.get('active_end_year');d=r['patch']['date'] if r['patch'] else r['before']
    if not p.get('source_id') or not isinstance(first,int) or not isinstance(last,int) or not 1000<=first<=last<=1970 or last-first>125:raise ValueError('Invalid documented activity')
    if (first,last)!=(d['first'],d['last']) or d['precision'] not in ('exact','circa','range','circa_range'):raise ValueError('Activity must match its dated source artwork')
   elif a['birth_year'] is None or a['death_year'] is None or not 0<=a['death_year']-a['birth_year']<=125:raise ValueError('Invalid new biography')
...
END $$;
DO $$ BEGIN
 IF EXISTS(SELECT 1 FROM r2_checked c WHERE c.p#>>'{artist,timeline_basis}'='activity' AND (
 coalesce(c.p#>>'{painter,source_id}','')='' OR
 (c.p#>>'{artist,active_start_year}')::int IS DISTINCT FROM coalesce((c.p#>>'{patch,date,first}')::int,(c.p#>>'{before,first}')::int) OR
 (c.p#>>'{artist,active_end_year}')::int IS DISTINCT FROM coalesce((c.p#>>'{patch,date,last}')::int,(c.p#>>'{before,last}')::int) OR
 coalesce(c.p#>>'{patch,date,precision}',c.p#>>'{before,precision}') NOT IN ('exact','circa','range','circa_range') OR
 (c.p#>>'{artist,active_start_year}')::int<1000 OR (c.p#>>'{artist,active_end_year}')::int>1970 OR
 (c.p#>>'{artist,active_end_year}')::int<(c.p#>>'{artist,active_start_year}')::int OR
 (c.p#>>'{artist,active_end_year}')::int-(c.p#>>'{artist,active_start_year}')::int>125))
 THEN RAISE EXCEPTION 'Activity lacks dated museum-object evidence'; END IF;
END $$;
...
END $$;
WITH added AS (
 INSERT INTO artists(slug,display_name,sort_name,normalized_name,birth_year,death_year,birth_display,death_display,timeline_start_year,timeline_end_year,timeline_display,timeline_basis,active_start_year,active_end_year,activity_display,status,created_by,updated_by)
 SELECT a->>'slug',a->>'display_name',a->>'sort_name',a->>'normalized_name',(a->>'birth_year')::int,(a->>'death_year')::int,
 a->>'birth_year',a->>'death_year',
 CASE WHEN a->>'timeline_basis'='activity' THEN (a->>'active_start_year')::int ELSE (a->>'birth_year')::int END,
 CASE WHEN a->>'timeline_basis'='activity' THEN (a->>'active_end_year')::int ELSE (a->>'death_year')::int END,
 CASE WHEN a->>'timeline_basis'='activity' THEN 'Documented work '||(a->>'active_start_year')||'–'||(a->>'active_end_year')||' (not lifespan)' ELSE (a->>'birth_year')||'–'||(a->>'death_year') END,
 coalesce(a->>'timeline_basis','life'),(a->>'active_start_year')::int,(a->>'active_end_year')::int,
 CASE WHEN a->>'timeline_basis'='activity' THEN 'Documented work '||(a->>'active_start_year')||'–'||(a->>'active_end_year')||' (not lifespan)' END,
 'review','local-european-research','local-european-research'
...
DO $$ BEGIN
 IF EXISTS(SELECT 1 FROM r2_ready v WHERE v.p#>>'{artist,timeline_basis}'='activity'
 AND NOT EXISTS(SELECT 1 FROM external_identifiers e WHERE e.entity_type='artist' AND e.entity_id=v.artist_id
 AND e.scheme=(v.p->>'source')||'-person' AND e.external_id=v.p#>>'{painter,source_id}'))
 THEN RAISE EXCEPTION 'Documented activity creator authority belongs to another painter'; END IF;
END $$;
WITH linked AS (
