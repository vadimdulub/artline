...
spec=importlib.util.spec_from_file_location('research',Path(__file__).with_name('research-armenian-georgian-artworks.py'))
bridge=importlib.util.module_from_spec(spec);spec.loader.exec_module(bridge)
r=bridge.r
core=r.core
ACTOR='local-european-research'
VERSION='armenian-georgian-selected-v1'
SOURCE_SLUG='armenian-georgian-catalogue-20260913'
SOURCE_NAME='Armenian and Georgian research — documented collection statements'
IMAGE_SOURCE_SLUG='armenian-georgian-images-20260913'
...
def plan(number,limit):
    ...
    if input_path.exists():candidates=json.loads(input_path.read_text())
        ...
        candidates=[json.loads(p.read_text()) for p in sorted((r.RUN/'final-ready').glob('Q*.json')) if p.stem not in prior][:limit]
        ...
        with r.base.connect(target=='production') as db,db.transaction():
            ...
            for item in candidates:
                ...
                if cachekey not in inventory_cache:
                    ...
                    titles=list({t.lower() for peer in peers for t in peer['titles']})
...
def artist_insert(db,author,record,sid):
    ...
    for word,code in [('Armenian','AM'),('Georgian','GE'),('Greek','GR'),('Russian','RU'),('French','FR'),('Italian','IT'),('Dutch','NL'),('Spanish','ES'),('German','DE'),('British','GB'),('American','US'),('Swedish','SE'),('Danish','DK'),('Norwegian','NO'),('Portuguese','PT')]:
        if re.search(r'\b'+word+r'\b',author['description'],re.I):
...
def apply(number,target,pin):
    ...
    assert json.loads((r.RUN/'backups.json').read_text())['production']['status']=='SUCCESSFUL'
        ...
        for state in plan_data['targets'][target]:
            ...
            with db.transaction(), db.pipeline():
                ...
                if state['action']=='new':
                    ...
                    description=f"{record['title']}, recorded by Wikidata as a work by {record['creator_label']} connected with {collection}. Metadata remains in review."
                    ...
                    assert work_type in ('painting','fresco','unknown'),'Source artwork type requires a dedicated adapter'
                    r.base.insert(db,'artworks',dict(id=aid,slug='wikimedia-artwork-'+qid.lower(),title=record['title'],normalized_title=r.norm(record['title']),date_display=d['display'],creation_year_start=d['first'],creation_year_end=d['last'],date_precision=d['precision'],work_type=work_type,object_form=record.get('source_form'),description_md=description,current_institution_id=state['institution_id'],current_location_text=collection,location_checked_at=record['entity_receipt']['retrieved_at'],accession_number=record['accession'],primary_media_id=media,status='review',created_by=ACTOR,updated_by=ACTOR,unlinked_creator_label=None if artist else record['creator_label'],research_candidate=d['precision']=='unknown'))
                    if artist:r.base.insert(db,'artwork_artists',dict(artwork_id=aid,artist_id=artist,attribution_role='primary',attribution_note='Unqualified Wikidata P170 creator statement; source evidence retained for editorial review.'))
                    r.base.insert(db,'artwork_location_assertions',dict(artwork_id=aid,claim_type='holding',institution_id=state['institution_id'],context='collection',source_id=sid,source_url='https://www.wikidata.org/wiki/'+qid,evidence_note='Wikidata best-ranked, non-expired P195 collection statement. Institution identity matched through official website or Muséofile ID. This collection claim remains in review; present display, ownership and visitor access are not asserted.',checked_at=record['entity_receipt']['retrieved_at'],review_state='review'))
                    db.execute("INSERT INTO curated_collections(institution_id,curator_kind,title,status) VALUES(%s,'owner','Selected research works','review') ON CONFLICT(institution_id,curator_kind) DO NOTHING",(state['institution_id'],))
                    cc=db.execute("SELECT id FROM curated_collections WHERE institution_id=%s AND curator_kind='owner'",(state['institution_id'],)).fetchone()['id']
                    db.execute("INSERT INTO curated_collection_items(collection_id,artwork_id,position,reason,source_id,source_url,checked_at) VALUES(%s,%s,(SELECT coalesce(max(position),0)+1 FROM curated_collection_items WHERE collection_id=%s),%s,%s,%s,%s) ON CONFLICT(collection_id,artwork_id) DO NOTHING",(cc,aid,cc,'Owner research selection: '+record['selection_note'],sid,'https://www.wikidata.org/wiki/'+qid,record['entity_receipt']['retrieved_at']))
                ...
                r.base.insert(db,'citations',dict(entity_type='artwork',entity_id=aid,field_name='wikimedia_catalogue_research',source_id=sid,source_record_id=qid,source_url='https://www.wikidata.org/wiki/'+qid,evidence_note='Wikidata CC0 identity, unqualified creator, collection and source date evidence. Existing catalogue titles, attribution and dating preserved. Entity capture SHA-256: '+record['entity_receipt']['sha256'],retrieved_at=record['entity_receipt']['retrieved_at'],created_by=ACTOR))
