...
s=importlib.util.spec_from_file_location('writer',ROOT/'ops/apply-expanded-activity.py');writer=importlib.util.module_from_spec(s);s.loader.exec_module(writer)
...
def audit(directory,label,before):
 ...
 for start in range(0,len(plan),250):
  ...
  SELECT jsonb_build_object('rid',p.k->>'rid','work',w.slug,'artist',a.slug,
   ...
   'artist_metadata',CASE WHEN a.id IS NULL THEN NULL ELSE jsonb_build_object('birth',a.birth_year,'death',a.death_year,'status',a.status,'basis',a.timeline_basis,'start',a.timeline_start_year,'end',a.timeline_end_year,'active_start',a.active_start_year,'active_end',a.active_end_year,'display',a.timeline_display) END,
 ...
 for row in data:
  ...
  if before:
   ...
   if a['new']:
    expected={'birth':a['birth_year'],'death':a['death_year'],'status':'review','basis':a.get('timeline_basis','life'),'start':a.get('active_start_year',a['birth_year']),'end':a.get('active_end_year',a['death_year']),'active_start':a.get('active_start_year'),'active_end':a.get('active_end_year'),'display':('Documented work '+str(a['active_start_year'])+'–'+str(a['active_end_year'])+' (not lifespan)') if a.get('timeline_basis')=='activity' else str(a['birth_year'])+'–'+str(a['death_year'])}
    assert row['artist_metadata']==expected,('new artist',r['rid'])
