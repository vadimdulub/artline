  ...
  return <>
    <section className="explorer" aria-labelledby="timeline-title">
      ...
      <div className="range-track">
        <button className="range-window" aria-label="Move selected time range" style={{ left: `min(${(start - 1100) / 9}%, calc(100% - 24px))`, width: `${(end - start) / 9}%` }} onPointerDown={event => { event.currentTarget.setPointerCapture(event.pointerId); drag.current = { x: event.clientX, start, end, width: event.currentTarget.parentElement!.clientWidth }; }} onPointerMove={event => { if (!drag.current) return; const d = drag.current; const offset = Math.round((event.clientX - d.x) / d.width * 900); const a = Math.max(1100, Math.min(2000 - (d.end - d.start), d.start + offset)); setRange(a, a + d.end - d.start); }} onPointerUp={() => { drag.current = null; }} onPointerCancel={() => { drag.current = null; }} onKeyDown={event => { if (event.key === "ArrowLeft" || event.key === "ArrowRight") { event.preventDefault(); pan(event.key === "ArrowLeft" ? -10 : 10); } }}><span>Drag to move the range</span></button>
