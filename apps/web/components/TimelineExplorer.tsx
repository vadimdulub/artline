      ...
      <div className="timeline-dark">
        ...
        <div className="timeline-stage" ref={stage} aria-busy={loading}>
          ...
          {error ? <div className="state-panel"><h2>We couldn’t load this view</h2><p>{error}</p><button onClick={() => { setResult({ key: "" }); setRetry(value => value + 1); }}>Try again</button></div> :
            data?.mode === "density" ? <><TimelineOverview key={`${start}-${end}`} periods={data.periods} start={start} end={end} disabled={loading} onSelect={setRange} /><div className="density-help"><p>Grouped by the midpoint of recorded life or activity dates. Choose a period to explore painters whose dates overlap it.</p><a href="/catalogue">Browse all painters</a></div></> :
