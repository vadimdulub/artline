...
function Lane({lane,range,bounds,width,busy,selected,select,narrow,browse,remove}:{lane:AtlasLane;range:BookRange;bounds:BookRange;width:number;busy:boolean;selected:string;select:(item:AtlasItem)=>void;narrow:(start:number,end:number,kind:AtlasType)=>void;browse:()=>void;remove:()=>void}){
 const marks=positionBooks(lane.items,bounds,width,linearFrom),height=Math.max(1,...marks.map(item=>item.lane+1))*72+8;
...
export function AllAtlas(){
 ...
 const ticks=bookAxisTicks(bounds,width,linearFrom);
 const band=preset&&windowKind==="context"&&!params.has("start")?{left:bookTickPosition(preset.period.start,bounds,linearFrom),right:bookTickPosition(preset.period.end,bounds,linearFrom)}:null;
 return <section ref={shell} className="explorer all-explorer" aria-labelledby="all-timeline-title">
  ...
  <div className={`timeline-dark${populated?"":" all-empty-canvas"}`}>
   ...
   {populated?<>
    ...
    <TimelineGrid stageRef={stage} busy={busy} selection={{left:bookTickPosition(displayRange.start,bounds,linearFrom),right:bookTickPosition(displayRange.end,bounds,linearFrom)}} ticks={ticks.map(t=>({key:t.year,label:t.label,position:bookTickPosition(t.year,bounds,linearFrom)}))}>
