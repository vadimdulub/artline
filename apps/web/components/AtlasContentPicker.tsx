...
export function AtlasContentPicker({ metadata, range, layers, initialType="artwork", browse=false, filterQuery, close, addLayer, open }: {
 ...
 if(!browse){const current=new URLSearchParams(filterQuery);for(const name of ["q","region","country","continent"])for(const value of current.getAll(name))search.append(name,value)}
 ...
 useEffect(()=>{const c=new AbortController();const timer=setTimeout(()=>apiRequest<AtlasResponse>(`atlas?${key}`,{signal:c.signal}).then(data=>{if(!c.signal.aborted)setResult({key,attempt:retry,data})}).catch(e=>{if(!c.signal.aborted)setResult({key,attempt:retry,error:errorMessage(e)})}),140);return()=>{clearTimeout(timer);c.abort()}},[key,retry]);
 return <RecordDrawer label={browse?`Browse ${definition.name.toLowerCase()}`:"Add a layer"} closeLabel="Close content picker" title={browse?definition.name:"Add a layer"} recordKey={type} close={close} fallbackFocusId="all-timeline-title">
  <div className={`atlas-picker${browse?"":" atlas-layer-picker"}`}>
   ...
   {browse?<>
    <div role="status">{busy?<LoadingIndicator label="Finding entries…"/>:error?"Count unavailable":`${lane?.total.toLocaleString("en-GB")??0} matching entries`}</div>
