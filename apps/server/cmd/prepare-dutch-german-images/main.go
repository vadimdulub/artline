// prepare-dutch-german-images downloads only a pinned, individually rights-reviewed
...
func validate(entries []map[string]any) error {
	if len(entries) < 1 || len(entries) > 40 {
		return errors.New("selection must contain 1..40 images")
	}
	seen := map[string]bool{}
	idRE := regexp.MustCompile(`^[a-f0-9-]{36}$`)
	for _, e := range entries {
		if text(e, "provider") == "denmark-switzerland-selected" || text(e, "provider") == "denmark-switzerland-followup" {
			if err := validateDKCH(e, seen); err != nil {
				return err
			}
			continue
		}
		...
		if !idRE.MatchString(id) || seen[id] || u.Scheme != "https" || !(u.Host == "cdn.thenetexperts.info" || u.Host == "search.smb.museum") || u.User != nil || u.RawQuery != "" || u.Fragment != "" || !((u.Host == "cdn.thenetexperts.info" && strings.HasPrefix(u.Path, "/image/authenticated/")) || (u.Host == "search.smb.museum" && strings.HasPrefix(u.Path, "/media/images/thumbs/extra_large/"))) {
			return errors.New("unapproved/duplicate image identity or URL")
		}
		seen[id] = true
		if text(e, "identity_basis") == "" || text(e, "creator_credit") == "" || text(e, "source_evidence_sha256") == "" || text(e, "provider") != "dutch-german-primary" {
		...
		if (u.Host == "cdn.thenetexperts.info" && (rights != "cc_by_sa" || uri != "https://creativecommons.org/licenses/by-sa/4.0/")) || (u.Host == "search.smb.museum" && (rights != "public_domain" || uri != "https://creativecommons.org/publicdomain/mark/1.0/")) {
			return errors.New("rights do not match reviewed museum workflow")
		}
...
func run(root, input, pin, out string) error {
	...
	for n, e := range entries {
		...
		if prior, err := os.ReadFile(receipt); err == nil {
			...
			p := text(old, "path")
			if !strings.HasPrefix(p, "/assets/artworks/imported/"+imagePrefix(e)+"-study-") || text(old, "selection_sha256") != pin {
		...
		original := filepath.Join("/Users/vadimdulub/Library/Application Support/Artline/source-images/"+imageSourceFolder(e), digest(raw)+".jpg")
		if err = writeNew(original, raw); err != nil {
			return err
		}
		e["source_original_path"] = original
		hash := digest(data)
		path := "/assets/artworks/imported/" + imagePrefix(e) + "-study-" + hash + ".jpg"
...
func main() {
	...
	sheet := flag.String("contact-sheet", "", "Create a contact sheet from prepared files only, without network")
	flag.Parse()
	if *input == "" || *out == "" {
	...
	if *sheet != "" {
		if err := contactSheet(*root, *input, *out, *sheet); err != nil {
